...
class InstanceViewSetTestCase(TestCase):
    ...
    def setUpTestData(cls):
        """Create some data."""
        factories.KalabashExtensionFactory(name="kalabash-amavis")
        factories.KalabashExtensionFactory(name="kalabash-stats")
        cls.md_instance = factories.KalabashInstanceFactory(
    ...
    def test_create(self):
        ...
        data.update({
            ...
            "extensions": ["kalabash-amavis"],
        })
        response = self.client.post(url, data=data, format="json")
        self.assertEqual(response.status_code, 201)
        instance = response.json()
        instance = models.KalabashInstance.objects.get(pk=instance["pk"])
        self.assertTrue(
            instance.extensions.filter(name="kalabash-amavis").exists())
    ...
    def test_update(self):
        ...
        data = {
            ...
            "domain_counter": 10, "extensions": [
                "kalabash-amavis", "kalabash_stats"]
        }
        ...
        self.assertTrue(
            self.md_instance.extensions.filter(
                name="kalabash-amavis").exists())
        self.assertTrue(
            self.md_instance.extensions.filter(
                name="kalabash-stats").exists())
        data["extensions"] = ["kalabash-amavis"]
        url = reverse("instance-detail", args=[self.md_instance.pk])
        response = self.client.put(url, data=data, format="json")
        self.assertEqual(response.status_code, 200)
        self.md_instance.refresh_from_db()
...
class VersionViewSetTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        factories.KalabashExtensionFactory(name="kalabash-amavis")
        factories.KalabashExtensionFactory(name="kalabash-stats")
        factories.KalabashExtensionFactory(name="kalabash-webmail")
...
class CurrentVersionAPI(TestCase):
    ...
    def test_current_version(self):
        ...
        self.assertTrue(
            models.KalabashInstance.objects.filter(
